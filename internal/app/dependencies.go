...
func newDependencies(cfg config.Config, db *gorm.DB) appDependencies {
	...
	return appDependencies{
		...
		financeHandler:            newFinanceHandler(db),
		stockCountHandler:         newStockCountHandler(db),
		creditSaleHandler:         newCreditSaleHandler(db),
		promotionHandler:          newPromotionHandler(db),
