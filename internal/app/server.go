...
func NewServer(cfg config.Config) (*Server, error) {
	...
	// Phase W1/W2: run provisioning in background so a stuck DB lock never
	// prevents the HTTP server from binding and serving traffic.
	go func() {
		provCtx, provCancel := context.WithTimeout(context.Background(), 60*time.Second)
		defer provCancel()
		provisioning.BackfillAllStores(provCtx, db)
		provisioning.BackfillProductDefaultLocations(provCtx, db)
	}()
